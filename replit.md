...
- **Offline Music**: Service worker buffers full audio → cache with range-request support → play offline with seek
- **Offline Library**: `/offline.html` — lists all cached tracks, play/delete per track, online/offline status bar
- **Offline Detection**: Main app shows red toast + "📴 OFFLINE LIBRARY" badge when internet drops
- **Cache Toast**: Green notification bottom-right when a track finishes caching (offline-ready)
- **Manifest**: `id`, `display_override: ["standalone"]`, shortcuts for Search + Offline Library
- **Icons**: 72/96/128/144/192/512px generated by jimp at startup
- **SW routes**: `/manifest.json`, `/sw.js`, `/icon-*.png`, `/offline.html` all served by index.js
